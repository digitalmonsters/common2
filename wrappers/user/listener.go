...
func (w *Wrapper) InitKafkaListener(kafkaConfig boilerplate.KafkaListenerConfiguration, ctx context.Context) {
...
func (w *Wrapper) mapKafkaMessages(messages []kafka.Message) ([]SimpleUser, []error) {
	eventsMap := make([]SimpleUser, len(messages))
	var appErrors []error

	for i, message := range messages {
		var event SimpleUser
